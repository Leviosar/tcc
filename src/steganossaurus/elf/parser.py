...
from steganossaurus.enums.elf import (
    BitFormat,
    MachineType,
    ProgramFlags,
    ProgramType,
...
def parse(
    filename, supported=["*"], debug=False
) -> Generator[Tuple[Instruction, int, int], None, None]:
    with open(filename, "rb+") as fp:
        ...
        for i in range(header["e_phnum"]):
            ...
            try:
                if (
                    ProgramType(program_header["p_type"]) == ProgramType.LOAD
                    and ProgramFlags(program_header["p_flags"]) == ProgramFlags.RX
                ):
                    logging.debug(f"Found code segment")
                    logging.debug(
                        f"Permissions: {ProgramFlags(program_header['p_flags'])}"
                    )
        ...
        string_table = StringTable(
            fp, string_table_header["sh_offset"], string_table_header["sh_size"]
        )
        ...
        for i in range(header["e_shnum"]):
            ...
            logging.debug(
                f"Section type: {SectionType.from_int(section_header['sh_type'])}"
            )
