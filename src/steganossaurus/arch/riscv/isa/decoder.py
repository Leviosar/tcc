...
class RiscVDecoder(Decoder):
    def decode(self, pointer: BufferedRandom) -> Tuple[Optional[Instruction], int]:
        ...
        if opcode_hint != "11":
            types = [CRType, CAType, CIType, CJType, CBType]
            ...
            for type in types:
                ...
                raise ValueError(
                    f"Unsupported compact instruction type: {binary} with opcde {opcode}"
                )
        else:
            ...
            types = [BType, IType, JType, RType, SType, UType, CType]
            ...
            for type in types:
                if opcode in type.opcodes:
                    try:
                        ...
                        print(
                            f"Unsupported instruction type: {binary} with opcde {opcode}"
                        )
            else:
                raise ValueError(
                    f"Unsupported instruction type: {binary} with opcde {opcode}"
